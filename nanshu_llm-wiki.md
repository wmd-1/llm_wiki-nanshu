...
用户选择模板后，后端 `create_project` 命令执行初始化：
...
### 2.2 两步思维链摄入（Two-Step Chain-of-Thought Ingest）
...
### 2.4 文档预处理
 ...
 后端 `preprocess_file` 命令将多种格式转为纯文本：
...
### 3.1 多阶段检索管线（Retrieval Pipeline）
...
### 3.2 四信号相关性模型（4-Signal Relevance Model）
...
## 十三、对 Karpathy LLM Wiki 模式的实现映射

> 本节将 Karpathy 在 `llm-wiki.md` 中提出的抽象模式，逐条映射到本项目的具体实现，说明每个理念是如何被工程化落地的，以及本项目在哪些地方做出了超越原始设计的扩展。

### 13.1 核心理念：从 RAG 检索到编译式知识管理

**原始理念**：传统 RAG 每次查询从头检索、从头合成，知识无法积累。LLM Wiki 的核心思想是让 LLM **增量构建并维护一个持久化 Wiki**——知识编译一次、持续更新，查询时直接引用已编译的结果。

**项目实现**：

- **持久化 Wiki 层**（第二节）：项目在 `wiki/` 目录下维护 7 种类型的 Markdown 页面（entities / concepts / sources / queries / comparisons / synthesis / overview），LLM 在摄入时生成并写入磁盘，跨会话持久存在
- **两步思维链摄入**（第二节 2.2）：不是查询时实时检索原始文档，而是预先将源文档"编译"为结构化 Wiki 页面。查询管线（第三节）直接在已编译的 Wiki 上操作，而非每次从原始文档重新推导
- **增量缓存**（第二节 2.2 Step 5）：SHA256 哈希比对，内容未变的源文档不重复处理，确保"编译一次"
- **`dataVersion` 机制**（第十一节）：单调递增版本号，Wiki 内容变更时 bump，驱动图缓存失效和 UI 响应式刷新——整个系统围绕"持久化、可复用"设计
...
| 原始层 | 实现方式 | 说明 |
|--------|----------|------|
| **Raw Sources** | `raw/sources/` 目录 | 原始文档不可变，LLM 只读不写。支持 PDF/DOCX/PPTX/XLSX 等格式自动预处理（第二节 2.4） |
| **The Wiki** | `wiki/` 目录 + 7 种页面类型 | LLM 完全拥有此层——创建、更新、交叉引用均由 LLM 驱动。用户通过 Obsidian 或应用内预览阅读（第二节 2.1） |
| **The Schema** | `schema.md` + `purpose.md` | 双文件配置：`schema.md` 定义页面类型、命名约定、frontmatter 格式、交叉引用规则；`purpose.md` 定义项目目标、关键问题、研究范围。LLM 在每次摄入和查询时读取两者 |

**扩展——Purpose.md**：原始设计只有 Schema（Wiki 如何运作），本项目额外引入 `purpose.md`（Wiki 为何存在），将结构规则与方向意图分离。LLM 读取 purpose.md 以保持知识积累的方向一致性——这是一个原始设计中未显式区分的维度。

### 13.3 三大操作

**原始理念**定义了三个核心操作：Ingest、Query、Lint。

#### Ingest（摄入）

**原始描述**：用户投入新源文档，LLM 读取、提取关键信息、写入摘要页、更新索引和实体/概念页、追加日志。单份源文档可能触及 10-15 个 Wiki 页面。

**项目实现**——两步思维链摄入（第二节 2.2）：

| 原始操作 | 实现机制 |
|----------|----------|
| 读取源文档 | Rust 后端预处理（PDF/DOCX/PPTX/XLSX → 文本）+ 前端读取 Markdown/文本 |
| 提取关键信息 | Step 1（Analysis）：LLM 提取实体、概念、论点、与已有知识的关联和矛盾 |
| 写入 Wiki 页面 | Step 2（Generation）：LLM 按 `---FILE---` 格式输出多个文件块，程序解析后写入磁盘 |
| 更新索引 | Step 2 自动生成更新后的 `wiki/index.md` |
| 更新实体/概念页 | Step 2 生成 entities/ 和 concepts/ 下的页面，含 `[[wikilink]]` 交叉引用 |
| 追加日志 | `wiki/log.md` 采用 append 模式写入，不覆盖历史 |
| 触及多页面 | 单次摄入通过 FILE 块机制一次性写入多个页面（实体页、概念页、来源摘要页、索引更新、日志追加、概览更新） |

**超越原始设计的扩展**：

- **摄入队列**（第二节 2.3）：串行化、磁盘持久化、崩溃恢复、自动重试——将原始设计中"人手动指示 LLM"的松散流程工程化为可靠的任务系统
- **REVIEW 块**（第二节 2.2 Step 4）：LLM 在摄入时主动标记矛盾、缺失页面、研究建议，形成异步审核队列
- **语言守卫**：自动检测 LLM 生成内容的语言，与用户设定不符的文件块被丢弃
- **来源摘要兜底**：若 LLM 未生成 source summary 页，程序自动用 Step 1 分析结果创建

#### Query（查询）

**原始描述**：LLM 搜索相关 Wiki 页面、阅读、合成答案、引用来源。好的答案可以回写为新的 Wiki 页面。

**项目实现**——多阶段检索管线（第三节）：

| 原始操作 | 实现机制 |
|----------|----------|
| 搜索相关页面 | Phase 1：分词搜索（英文 whitespace + 中文 CJK bigram）+ Phase 1.5：可选向量语义搜索（LanceDB） |
| 扩展上下文 | Phase 2：知识图谱 1-hop 扩展，以搜索结果为种子沿相关性边扩展 |
| 合成答案 | Phase 3-4：预算控制 + 上下文组装，按优先级（标题匹配 > 内容匹配 > 图谱扩展 > 概览兜底）填入，LLM 流式生成带编号引用的答案 |
| 答案回写 Wiki | "Save to Wiki" 功能：将对话中 LLM 的输出保存到 `wiki/queries/`，再触发摄入 |

**超越原始设计的扩展**：

- **预算控制**（第三节 3.1 Phase 3）：根据用户配置的上下文窗口大小按比例分配 token 预算，适配从 4K 到 1M 字符的不同模型
- **引用追踪**：每条 AI 回复保存 `references: MessageReference[]`，记录引用的 Wiki 页面
- **多会话管理**：独立 Conversation 对象，各自的消息列表和历史记录

#### Lint（健康检查）

**原始描述**：定期检查矛盾、过时声明、孤立页面、缺失页面、缺失交叉引用、可填补的知识空白。

**项目实现**——双模式 Lint（第七节）：

| 原始检查项 | 实现机制 |
|-------------|----------|
| 孤立页面 | Structural Lint：Orphan 检测（无入链页面） |
| 缺失交叉引用 | Structural Lint：No-outlinks 检测（无出链页面）+ Broken-link 检测（指向不存在页面的 `[[wikilink]]`） |
| 矛盾 | Semantic Lint：LLM 通读页面摘要，检测跨页面矛盾 |
| 过时声明 | Semantic Lint：LLM 检测被新信息取代的旧声明 |
| 缺失页面 | Semantic Lint：LLM 检测高引用但无专页的概念 |

**超越原始设计的扩展**：

- **知识图谱洞察**（第四节 4.3）：Surprising Connections（惊奇连接）和 Knowledge Gaps（知识空白：孤立节点、稀疏社区、桥接枢纽节点）提供超越文本 Lint 的结构级诊断
- **审核系统自动清理**（第六节 6.2）：Sweep 机制在摄入队列排空后自动处理已解决的审核项——规则匹配优先（零 token），LLM 判断兜底（有限 token）

### 13.4 索引与日志

**原始理念**：

- `index.md`：内容导向的目录，每个页面列出链接 + 一行摘要 + 可选元数据，LLM 每次摄入时更新，查询时先读索引再深入
...
| 文件 | 实现方式 | 与原始设计的关系 |
|------|----------|------------------|
| `wiki/index.md` | 按页面类型分组的目录，每项 `[[page-slug]] — 一行描述` 格式。Step 2 生成时自动更新。查询管线 Phase 3 分配 5% 预算专门加载索引 | 完全实现，并工程化为查询管线的固定环节 |
| `wiki/log.md` | 逆时间顺序追加，格式 `## YYYY-MM-DD`，追加模式写入（不覆盖历史） | 完全实现。语言守卫对 log.md 豁免语言检测，因为日志天然包含跨语言专有名词 |

### 13.5 交叉引用与 Wikilink

**原始理念**：Wiki 的核心价值在于交叉引用——页面之间的连接让知识成为网络而非孤岛。原始设计依赖 LLM 在生成时主动添加 `[[wikilink]]`，但未深入讨论链接质量。

**项目实现**：

- **Wikilink 丰富化**（第十节）：针对"LLM 重写页面添加链接时会趁机改写内容"的问题，设计为 LLM 只返回替换清单（`{"links": [{"term": "...", "target": "..."}]}`），程序在原文中做安全替换——保证 byte-level 内容不变，仅插入 `[[` 和 `]]` 符号
- **Wikilink 解析**：大小写不敏感，空格与连字符兼容（`resolveTarget`），确保不同风格的链接都能正确解析
- **四信号相关性模型**（第三节 3.2）：Direct Link 信号（权重 ×3.0）将 wikilink 拓扑结构纳入页面相关性计算

### 13.6 知识图谱

**原始理念**：Obsidian 的 Graph View 是查看 Wiki 形状的最佳方式——什么连接什么、哪些是枢纽、哪些是孤立页。原始设计仅依赖 Obsidian 的内置图谱可视化。

**项目实现**——自建知识图谱引擎（第四节）：

| 原始能力 | 实现扩展 |
|----------|----------|
| 图谱可视化 | 内置 Graph View 组件 + Obsidian 兼容（`.obsidian/` 目录） |
| 识别枢纽/孤立页 | Graph Insights 自动检测孤立节点（degree ≤ 1）、桥接枢纽（连接 ≥ 3 个社区）、稀疏社区（cohesion < 0.15） |
| — | Louvain 社区检测：自动发现知识聚类，原始设计中未涉及 |
| — | 四信号相关性模型：Direct Link + Source Overlap + Adamic-Adar + Type Affinity，量化页面间关系强度 |
| — | Surprising Connections：检测跨社区、跨类型、边缘-枢纽等"意外"连接 |
| — | 图谱缓存（`dataVersion`）：Wiki 内容不变时复用已构建的图，避免重复计算 |

### 13.7 模板系统

**原始理念**：LLM Wiki 可应用于多种场景——研究、读书、个人成长、商业等，但原始设计未提供具体的场景化配置。

**项目实现**：5 种预设模板（第二节 2.1），每种模板预配置：

| 模板 | 额外目录 | 定制化配置 |
|------|----------|------------|
| Research | `wiki/methodology/`, `wiki/findings/`, `wiki/thesis/` | Schema 增加thesis/methodology/finding 类型及 frontmatter 字段；Purpose 聚焦研究问题、假说、方法论 |
| Reading | `wiki/characters/`, `wiki/themes/`, `wiki/plot/` | Schema 增加character/theme/plot 类型；Purpose 聚焦阅读目标、关注点 |
| Personal Growth | `wiki/journal/`, `wiki/habits/`, `wiki/goals/` | Schema 增加journal/habit/goal 类型；Purpose 聚焦个人目标和追踪维度 |
| Business | `wiki/stakeholders/`, `wiki/metrics/`, `wiki/decisions/` | Schema 增加stakeholder/metric/decision 类型；Purpose 聚焦业务目标和关键指标 |
| General | 无额外目录 | 最小化配置，通用起点 |

这是将原始设计中"不同领域需要不同配置"的理念工程化为可即用的模板系统。

### 13.8 工具链扩展

**原始理念**：小规模用 index.md 即可搜索，规模增长后需要搜索引擎（如 qmd）。可选 CLI 工具辅助 LLM 更高效操作 Wiki。

**项目实现**：

| 原始提及 | 实现方式 |
|----------|----------|
| 搜索引擎 | 内建分词搜索（Phase 1）+ 可选向量语义搜索（Phase 1.5, LanceDB），从百页到千页规模均可用 |
| Obsidian Web Clipper | Chrome 扩展（第九节）：Readability.js 正文提取 + Turndown.js HTML→Markdown + 项目选择器 + Clip Server 接收 + Clip Watcher 自动摄入 |
| 本地图片下载 | `raw/assets/` 目录 + 预处理管线支持图片格式 |
| Obsidian 图谱 | 内置 Graph View + `.obsidian/` 兼容目录 |
| Git 版本管理 | 项目目录为标准文件系统结构，天然支持 Git |

**超越原始设计的扩展**：

- **Deep Research**（第五节）：原始设计未涉及的网络搜索闭环——Tavily API 搜索 → LLM 合成带 `[[wikilink]]` 的研究页面 → 自动摄入回知识库，实现"研究反哺知识库"
- **多格式文档预处理**（第二节 2.4）：Rust 后端原生支持 PDF/DOCX/PPTX/XLSX/图片等格式，原始设计未涉及非文本文档
- **Review 系统**（第六节）：结构化审核队列 + 自动清理（Sweep），在原始设计"人参与循环"的基础上增加异步审核和自动解决机制

### 13.9 "LLM 即维护者"哲学的工程化

**原始理念**：人类放弃 Wiki 的原因是维护负担增长快于价值。LLM 不会厌倦、不会忘记更新交叉引用、可以一次触及 15 个文件。人的工作是策展来源、引导分析、提出好问题；LLM 的工作是一切其他。

**项目实现——系统级保障**：

| 原始哲学 | 工程化机制 |
|----------|------------|
| LLM 不会忘记更新交叉引用 | Wikilink Enrichment（第十节）：安全替换机制保证链接被添加且原文内容不变 |
| LLM 可以一次触及 15 个文件 | FILE 块批量写入（第二节 2.2 Step 3）：单次摄入可同时写入实体页、概念页、来源摘要、索引更新、日志追加、概览更新 |
| 维护成本趋近于零 | 自动化管线：摄入队列 → 两步摄入 → Review 生成 → Sweep 自动清理 → dataVersion 刷新 |
| 人负责策展和引导 | 审核系统（第六节）：LLM 生成审核项供人决策，但日常维护（更新索引、补充交叉引用、标记矛盾）完全由 LLM 完成 |
| 知识持续积累 | 增量缓存（避免重复处理）+ 向量嵌入（提升检索）+ 图谱社区检测（发现知识聚类）——三层机制确保知识库随使用而增值 |

### 13.10 实现总览

```
Karpathy 的抽象模式                    本项目的工程化实现
─────────────────                    ──────────────────

Raw Sources 不可变层          →      raw/sources/ + Rust 多格式预处理

The Wiki LLM 生成层           →      wiki/ 7 种页面类型 + 两步思维链摄入
                                      + FILE 块批量写入 + 语言守卫

The Schema 规则层             →      schema.md + purpose.md（新增方向意图层）

Ingest 操作                   →      串行摄入队列 + 两步 CoT + Review 块
                                      + 增量缓存 + 可选向量嵌入

Query 操作                    →      4 阶段检索管线（分词→向量→图谱→预算）
                                      + 引用追踪 + 答案回写

Lint 操作                     →      结构化 Lint（规则）+ 语义 Lint（LLM）
                                      + 图谱洞察（社区/空白/惊奇连接）

index.md 目录                  →      wiki/index.md + 查询管线固定加载

log.md 日志                    →      wiki/log.md 追加模式 + 语言豁免

Obsidian 兼容                 →      .obsidian/ 目录 + [[wikilink]] 格式
                                      + 内置 Graph View 组件

Web Clipper                   →      Chrome 扩展 + Clip Server + 自动摄入

（未提及）                     →      Deep Research 网络搜索闭环
                                      + Review 审核系统 + Sweep 自动清理
                                      + 5 种场景模板 + 多会话聊天
                                      + 预算控制 + 向量语义搜索
```

核心判断：本项目忠实地实现了 Karpathy LLM Wiki 模式的全部核心理念——三层架构、三大操作、索引与日志、交叉引用、图谱可视化，并在每个环节进行了深度工程化，将"人与 LLM 协作维护知识库"的抽象模式转化为可运行的跨平台桌面应用。同时通过 purpose.md 方向意图层、Deep Research 闭环、审核系统、模板系统等扩展，在原始设计的基础上增加了可操作性、可靠性和场景适应性。
